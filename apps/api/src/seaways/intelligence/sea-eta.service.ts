...
import { VoyageMovementEtaResponse } from '@nexus-ways/shared';

@Injectable()
export class SeaEtaService {
  private readonly logger = new Logger(SeaEtaService.name);

  constructor(private readonly supabase: SupabaseService) {}

  /**
   * Calculates ETA confidence band for a voyage movement.
   * Groups historical movements by voyage_id directly.
   * - If >= 3 completed movements exist on the voyage: historical mean duration + stddev (confidence_basis: 'historical')
   * - If < 3 completed movements: default +/- 15% band (confidence_basis: 'default')
   */
  async calculateVoyageEta(
    orgId: string,
    movementId: string,
  ): Promise<VoyageMovementEtaResponse> {
  ...
  async calculateMovementEta(
    ...
    // 1. Fetch the target voyage movement
    ...
    if (movErr || !movement) {
      throw new NotFoundException(`Voyage movement ${movementId} not found`);
    }

    const voyageId = movement.voyage_id;
    const baseDuration = Number(movement.duration_minutes || movement.predicted_duration_minutes || 180);
    const distanceKm = Number(movement.distance_km || 450);

    // 2. Query completed historical movements for the exact same voyage_id
